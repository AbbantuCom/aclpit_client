...
│                      │                                          │                       │
│                      │ ◀─────────────────────────────────────── │                       │
└─────────────────────┘   POST /api/revalidate (on content save)  └──────────────────────┘
```
...
- **On-demand revalidation** — when an editor saves a section in the admin panel, `aclpit_admin_api` POSTs `{ secret, tag }` to this site's `app/api/revalidate/route.ts`, which calls `revalidateTag(tag, { expire: 0 })` to drop the cached copy immediately instead of waiting out the hourly window.
...
│   ├── layout.tsx              ← Navbar/Footer/CookieBanner/RevealObserver/QueryProvider
...
2. Set this repo's env vars: `ADMIN_API_URL` / `NEXT_PUBLIC_ADMIN_API_URL` pointing at that deployment, and `REVALIDATE_SECRET` matching its value there.
3. In `aclpit_admin_api`, set `CLIENT_URL` to this site's deployed URL (for the revalidate webhook) and add this site's origin to `CLIENT_ORIGIN` (for CORS on content GET + contact POST).
